...
public class TwitterService {
	...
	private void init() {
		...
		APP_ACCOUNT_SCREENNAME = PROP.getScreenname();// a remplacer par
		cb.setPrettyDebugEnabled(true);
		cb.setOAuthConsumerKey(PROP.getConsumerKey());// API key
		cb.setOAuthConsumerSecret(PROP.getConsumerSecret());// API
															// secret
		cb.setOAuthAccessToken(PROP.getAccesToken());
		cb.setOAuthAccessTokenSecret(PROP.getTokenSecret());
	...
	public Properties load(String filename) {
		...
		if (input != null) {
			...
			LOGGER.log(
					Level.SEVERE,
					"Le fichier de configuration de l'application  'WEB-INF/tweetbot.properties' est introuvable");
		}
	...
	public List<Long> getFollowersIDList(String screenName) {
		LOGGER.log(Level.INFO, "Chargement des id des followers du user : "
				+ screenName);
		IDs result = null;
